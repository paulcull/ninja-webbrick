  ...
  case "dimmer":
      ...
      if (state.bri > 0 && state.on) {
        ...
        wbd.dimmerLevel(this.wbOpts.channel,wbBri,function(err,value) {
          if (err) {
           self._logger.error('(WebBrick) Error in get set dimmer state on at %s for %s : %s. %s',wbBri, wbOpts.brickIp, wbOpts.channel, err);
          }
          this.emit('data',data)
        });
      } else {
        ...
        wbd.dimmerLevel(this.wbOpts.channel,0,function(err,value) {
          if (err) {
           self._logger.error('(WebBrick) Error in get set dimmer state off for %s : %s. %s',wbOpts.brickIp, wbOpts.channel, err);
          }
          this.emit('data',data)
        });        
      }
  ...
  case "button":
      ...
      this._logger.debug('(WebBrick) about to push button %s:%s',this.wbOpts.brickIp, this.wbOpts.channel);
        wbd.DigInTrigger(this.wbOpts.channel,function(err,value) {
          if (err) {
           self._logger.error('(WebBrick) Error in get push button for %s : %s. %s', wbOpts.brickIp, wbOpts.channel, err);
          }
          this.emit('data',data)
        });
    break;
