...
function Device(app, wbOpts, connPool) {
  ...
  var wbd = new WebBrick.createClient(wbOpts,app,connPool);
  this.wbd = wbd;


  var checkDevice = function (timer) {
      ...
      switch (wbOpts.deviceType) {      
        case "temp":
          wbd.tempState(wbOpts.channel,function(err,value) {
              if (!err) {
                ...
                self._app.log.debug('(WebBrick) Error in get Temp state for %s : %s. %s',wbOpts.brickIp, wbOpts.channel, err);
              }
            });
          break;
        case "pir":
        ...
        //do DO state stuff
          wbd.DigOutState(wbOpts.channel,function(err,value) {
              if (!err) {
                ...
                self._app.log.debug('(WebBrick) Error in get PIR state for %s : %s. %s',wbOpts.brickIp, wbOpts.channel, err);
                //self.emit('data','Off');
              }
            });
          break;
        ...
        case "dimmer":
          wbd.dimmerState(wbOpts.channel,function(err,value) {
              ...
              if (!err) {
                ...
                self._app.log.debug('(WebBrick) Error in get Dimmer state Light for %s : %s. %s.',wbOpts.brickIp, wbOpts.channel, err);
              }
            });
          break;
        case "text":
          wbd.createHeart(wbOpts.channel,function(err,value) {
              if (!err) {
                ...
                self._app.log.debug('(WebBrick) Error in creating Heartbeat for %s : %s. %s.',wbOpts.brickIp, wbOpts.channel, err);
        ...
        setTimeout(checkDevice,timer);
    };


  //process.nextTick(function() {
  //var timerId = setInterval( , this._timerInterval);
  checkDevice(this._timerInterval);
};
...
Device.prototype.write = function(data) {
  ...
  case "dimmer":
      ...
      try {
        ...
        this._app.log.error('(WebBrick) Error in parsing dimmer event object');
        return;
      }
