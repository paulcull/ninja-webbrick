...
Device.prototype.write = function(data) {
  ...
  case "text_command":
      ...
      wbd.command(data,function(err,value) {
        if (err) {
    ...
    break;
    case "text":
      this._logger.debug('(WebBrick) about to send command from text %s : %s',this.wbOpts.brickIp, data);
      wbd.command(data,function(err,value) {
        if (err) {
         self._logger.error('(WebBrick) Error in get send command from text on at %s : %s',data,  err);
        }
        //this.emit('data',data)
      });
    break;
